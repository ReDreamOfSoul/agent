...
from ..db.models import Interaction as DBInteraction, User
...
def chat(req: ChatRequest, workflow: WorkflowEngine = Depends(get_workflow),
         ...
         user: User | None = Depends(get_current_user_optional)) -> ChatResponse:
    ...
    if user:
        try:
            ...
            db.add(DBInteraction(
                session_id=result.session_id, user_id=user.id,
                intent=result.intent, question=req.question, answer=result.answer,
                hit=result.hit, best_score=result.best_score,
                generation_mode=result.generation_mode, retrieval_mode=result.retrieval_mode,
                followup_question=result.followup_question,
            ))
            db.commit()
            db.close()
            logger.info("对话记录已写入: user=%s, session=%s", user.username, result.session_id)
        except Exception as exc:  # noqa: BLE001
...
def chat_stream(req: ChatRequest, workflow: WorkflowEngine = Depends(get_workflow),
                sessions: SessionManager = Depends(get_sessions),
                user: User | None = Depends(get_current_user_optional)) -> StreamingResponse:
