...
from .api import admin, assignments, auth, chat, course, digital_teacher, exercise, feedback, graph, homework, insight, sessions, student, study, videos
...
def build_app() -> FastAPI:
    ...
    async def lifespan(app: FastAPI):
        # ---- 启动装配 ----
        # 初始化数据库（创建表 + 种子数据，已存在则跳过）
        try:
            from .db.init_db import init_db
            init_db(reset=False)
        except Exception as exc:  # noqa: BLE001
            logger.warning("数据库初始化跳过: %s", exc)
    ...
    app.include_router(digital_teacher.router)
