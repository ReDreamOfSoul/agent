...
成都理工大学 2025 年度 “AI 研究基金” 项目（**2025AI068**，AI + 人才培养）

**AI 赋能机器人学课程的教学改革和实践探索** —— 课程智能体开发代码。
...
* **知识库**：解析教材 PDF、十章课件（PPT/PPTX）、题库、教学大纲（DOC/DOCX），标题感知分段、重叠分块、章节打标；

* **检索**：BM25 关键词检索 + 可选向量检索（OpenAI 兼容 / 本地 Embedding）混合检索、阈值命中判定、归一化重排；
...
* **MCP 协议**：course\_kb\_retrieval /exercise\_judge/study\_record /graph\_query 四个工具按统一 MCP 协议注册，提供 JSON-RPC over stdio 服务端；
...
│   ├── build\_kb.py            # 知识库索引构建

│   ├── gen\_video\_manifest.py  # 扫描知识库 video/ 生成视频清单（可人工补充标题/描述）

│   └── test\_agent.py          # 命令行联调

├── tests/

│   ├── test\_workflow.py       # 工作流单元测试（无需真实数据/模型）

│   └── test\_agent\_modules.py  # 习题解析/知识图谱/学情画像/导出 单元测试

└── data/

&#x20;   ├── knowledge\_base/        # 知识库源文件（教材/课件/题库/大纲/video，放入此处）
...
| ------ | ------------------------------------------ |
| 操作系统   | Windows / Linux                            |
| Python | 3.10 及以上（本地 Embedding 模型建议 Python 3.10 环境） |
...
&#x20;   ├── ch06\_1.mp4               # 建议命名 ch{章节}\_{序号}.mp4，自动识别所属章节

&#x20;   └── manifest.json            # 视频清单（python scripts/gen\_video\_manifest.py 生成，可人工编辑标题/描述/时长）
```
...
```
python scripts/build\_kb.py            # 首次构建

python scripts/build\_kb.py --rebuild  # 强制重建

python scripts/build\_kb.py --no-ocr   # 跳过扫描版PDF的OCR（快速试跑）
```

> 扫描版教材（无文字层 PDF）自动启用 OCR 识别（RapidOCR），识别结果缓存在
> `data/cache/`
>
> ，同一 PDF 只需识别一次，之后构建直接复用缓存。
> 未配置向量服务时，自动使用 
>
> **BM25 关键词检索**
>
...
LLM\_API\_BASE\_URL=https://dashscope.aliyuncs.com/compatible-mode/v1

LLM\_API\_KEY=sk-xxxx

LLM\_MODEL=qwen-plus

\# Embedding 向量模型（配置后自动启用"向量+BM25"混合检索）

EMBEDDING\_API\_BASE\_URL=https://dashscope.aliyuncs.com/compatible-mode/v1

EMBEDDING\_API\_KEY=sk-xxxx

EMBEDDING\_MODEL=text-embedding-v3
```
...
```
python scripts/test\_agent.py "请介绍一种机器视觉滤波去噪算法"   # 单次问答

python scripts/test\_agent.py --chat                            # 交互式问答

python scripts/test\_agent.py --homework "什么是齐次变换矩阵" "我的答案"  # 作业批阅
```
...
| GET    | /v1/graph?node=…\&relation\_type=… | 知识点关系查询（prerequisite/successor/objective）            |
| GET    | /v1/exercises?limit=5              | 习题自测抽题（题库优先，不足时 AI 生成）                               |
| GET    | /v1/self-assessment?session\_id=…  | 学生个人学习画像（命中率 / 薄弱点 / 待订正）                            |
| GET    | /v1/recommend?session\_id=…        | 个性化学习推荐（知识补强 / 习题订正 / 学习建议）                          |
...
遵循统一 MCP 协议（JSON-RPC 2.0 over stdio）：
...
1. **MCP 工具方式**：将本服务作为 MCP Server 接入学校智能体开发平台，平台流程编排节点通过 `tools/call` 调用上述工具；

2. **HTTP 接口方式**：平台直接调用 5.1 节 REST 接口完成对话与批阅。
...
| 关键词阈值     | 0.5               | 纯关键词（未配置 Embedding）模式的命中阈值（idf 加权覆盖率语义）     |
| 混合权重      | 向量 0.6 / 关键词 0.4  | 配置向量服务后生效                                   |
| 分块大小 / 重叠 | 500 / 60 字符       | 标题感知分段 + 重叠分块                               |
| 扫描版 OCR   | 自动启用              | 无文字层 PDF 页经 RapidOCR 识别（结果缓存于 data/cache）   |
| 会话上下文     | 最近 10 轮 / 30 分钟超时 | 多轮对话管理                                      |
| 生成温度      | 0.3               | 保证回答稳定准确                                    |
...
| ------ | --------------------------------------------------------------------------- |
| 项目代码   | 本目录全部代码                                                                     |
| 模型文件   | 本地 Embedding 模型文件（sentence-transformers 缓存，可选）；配置见 embedding.provider=local |
